...
/**
 * Find free 30-min (or custom) slots using FreeBusy combined with a business-hours window.
 * @param {Object} params
 * @param {string} params.calendarId
 * @param {string} params.fromISO - ISO string UTC
 * @param {string} params.toISO   - ISO string UTC
...
async function findSlots({ calendarId, fromISO, toISO, tz, durationMin, maxSlots }) {
  ...
  while (cursor.isBefore(end) && slots.length < maxSlots) {
    ...
    if (dayEnd.isAfter(dayjs(fromISO))) {
      ...
      // Fill remaining time after the last busy block until dayEnd
...
app.post("/book", requireSecret, async (req, res) => {
  try {
    ...
    if (!calendarId || !start || !end) {
      return res
        .status(400)
        .json({ error: "calendarId, start, end are required" });
    }
